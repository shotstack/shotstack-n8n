...
| `nodes/Shotstack/Shotstack.node.ts` | the node type `shotstack`; the behaviour of version 1, which n8n stamps on every saved node as typeVersion; the field `resource`; the values `render`, `asset` and `reference` |
| `nodes/Shotstack/resources/render/index.ts` | the field `operation`; the values `postRender`, `postTemplateRender` and `getRender` |
| `nodes/Shotstack/resources/asset/index.ts` | the field `operation`; the values `getAssetByRenderId` and `download` |
...
| `nodes/Shotstack/resources/reference/index.ts` | the field `operation` and the value `getReference`; the fields `detail` and `includeTemplates`; the values `core`, `full` and `everything`; the output keys `reference`, `rulesSource`, `referenceChars`, `templates`, `templateCount`, `credentialError`, `documentation`, `documentationChars` and `documentationError` |
| `nodes/Shotstack/reference/skill.ts` | the keys a reader gets under `rulesSource`: `repo`, `ref`, `license` and `url`. Generated by `scripts/vendor-skill.mjs`, so edit the generator |
...
| `nodes/Shotstack/resources/asset/download.ts` | the fields `fileUrl` and `fileName`; the binary key `data`, which the next node names as its input field |
| `scripts/build-user-agent.mjs` | the product token `shotstack-n8n-node`. It writes `nodes/Shotstack/userAgent.ts`, so edit the generator, never the file. The version after the slash is meant to move |
