...
- **Never edit `nodes/Shotstack/userAgent.ts`.** It is generated by
  `scripts/build-user-agent.mjs`, which `npm run build` runs first, so an edit
  is lost on the next build or fails CI. Change the generator instead.
