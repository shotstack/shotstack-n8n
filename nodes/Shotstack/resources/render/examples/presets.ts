...
// `satisfies` rather than a type annotation: it still checks every value is an
// IDataObject, but keeps the key literals, so a renamed or mistyped key in
// renderFromExample.ts becomes a compile error instead of an undefined body.
export const EXAMPLE_EDITS = {
	...
	infoNewsSummaryVideo: {
	  ...
	  "timeline": {
	    "soundtrack": {
	      "src": "https://shotstack-assets.s3.amazonaws.com/music/unminus/kring.mp3",
	      "effect": "fadeOut"
	    },
	    "fonts": [
	      {
...
} satisfies Record<ExampleKey, IDataObject>;
